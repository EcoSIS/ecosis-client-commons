...
import render from "./app-virtual-list.tpl.js"

export default class AppVirtualList extends LitElement {
  ...
  updated(prop) {
    this.scrollPanel.style.height = (this.items.length*this.itemHeight)+'px';
    this._layoutItems();
  }
  ...
  _layoutItems() {
    ...
    for( let item of this.renderedItems ) {
      item.ele.setItem(item.index, this.items[item.index]);
      item.ele.style.top = (item.index*this.itemHeight)+'px';
      item.ele.style.left = '5px';
      item.ele.style.right = '5px';
    }
  }

}

customElements.define('app-virtual-list', AppVirtualList);
